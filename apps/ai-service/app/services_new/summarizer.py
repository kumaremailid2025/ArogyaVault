    ...
    *,
    ...
    if _HAS_OPENAI:
        try:
            ...
            summary_prompt = [
                SystemMessage(content="You are a medical community assistant. Summarise the discussion thread concisely in 2-3 sentences. Focus on consensus, actionable insights, and any open questions."),
                HumanMessage(content=f"Post: {post_text}\n\nReplies:\n{reply_block}"),
            ]
            summary_resp = await llm.ainvoke(summary_prompt)

            ai_prompt = [
                SystemMessage(content="You are a knowledgeable health AI. Provide a brief, evidence-based response to the health topic. Keep it factual and add practical advice. 2-3 sentences max."),
