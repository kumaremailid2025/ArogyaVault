 ...
 * Files Layout
 * ────────────
 * Two-column structure for the files section.
 * LEFT: FilesContainer with file list and search
 * RIGHT: Dynamic panel page (default view or file detail)
 *
 * Provides FilesContext to child pages so they can access files data
...
export default function FilesLayout({ children }: FilesLayoutProps) {
  ...
  /* ── Hardcoded variant and group ── */
  const variant = "community";
  ...
  const handleSelectFile = React.useCallback((fileId: number) => {
    ...
    router.push(`/community/files/${fileId}`);
  }, [router]);
  ...
  const contextValue = React.useMemo(
    () => ({
      variant: variant as "community" | "invited",
      group,
    ...
    [variant, group, communityFiles, selectedFileId, currentRecentQA, handleAskFileQuestion],
  );
