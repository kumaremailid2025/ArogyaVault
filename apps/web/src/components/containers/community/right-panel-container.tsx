...
/* ═══════════════════════════════════════════════════════════════════
   RIGHT PANEL CONTAINER — unified right sidebar for community
   Handles: default (analytics / connection-info), summary,
   replies, reply-preview. Driven by `variant` + `panelState`.
═══════════════════════════════════════════════════════════════════ */
...
interface RightPanelContainerProps {
  ...
  onSubmitReply: () => void;
  /** For community variant: unused. For invited variant: summary text */
  linkedSummary?: string;
  /** For community variant: unused. For invited variant: AI response text */
...
const getHasNative = (pendingReply: ComposeSubmitPayload | null): boolean =>
  pendingReply?.voiceRecording !== null &&
  pendingReply?.voiceRecording !== undefined &&
  !pendingReply.voiceRecording.lang.startsWith("en");

const getVoiceLangInfo = (pendingReply: ComposeSubmitPayload | null) => {
        ...
        {panelState.view === "replies" && activePost && (
          <div className="p-4 space-y-4">
            ...
            <div className="pt-1 border-t border-border">
              <p className="text-[11px] font-semibold text-muted-foreground pt-2 pb-2">
                Add your reply
              </p>
              <ComposeBox
        ...
        {panelState.view === "reply-preview" && activePost && panelState.original && panelState.rephrasings && (
          <div className="p-4 space-y-4">
            ...
            <div>
              ...
              >
                <p className="text-xs leading-relaxed">{panelState.original}</p>
                {selectedVersion === 0 && (
                    ...
                    <div className="flex items-center gap-3 bg-red-50 px-3 py-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold truncate">
                          {pendingReply.attachedDoc.filename}
                        </p>
                        <p className="text-[10px] text-muted-foreground">
                          {pendingReply.attachedDoc.docType}
                        </p>
                      </div>
                    </div>
                  ) : (
            ...
            {aiPerspective && (
              <div className="rounded-xl border border-amber-200 bg-amber-50/50 p-3">
                ...
                <p className="text-xs leading-relaxed text-foreground/80">{aiPerspective}</p>
              </div>
            )}
